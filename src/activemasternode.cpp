...
{    
    ...
    if(status != ACTIVE_MASTERNODE_STARTED) {
	    ...
	    std::cout << "[+] service = CService() " << std::endl;
	    std::cout << "[+] " << strMasterNodeAddr << std::endl;
            service = CService(strMasterNodeAddr);
        }

        if(Params().NetworkID() == CBaseChainParams::MAIN) {
            if(service.GetPort() != 7999) {
		std::cout << "[+] PORT NUMBER " << service.GetPort() << std::endl;
                notCapableReason = strprintf("Invalid port: %u - only 9999 is supported on mainnet.", service.GetPort());
                LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason);
                return;
            }
        } else if(service.GetPort() == 7999) {
